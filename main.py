...
#     app.run(debug=True)
#source code chỉnh sửa cho việc test
# package importing
from flask import Flask, make_response, request, render_template, url_for, redirect
import io
import csv
...
# Tải mô hình đã huấn luyện bằng joblib (thay vì pickle)
...
# Kiểm tra các cơ sở dữ liệu trong MySQL
mycursor.execute("SHOW DATABASES")
for x in mycursor:
    print(x)

# Chức năng chuyển đổi nội dung file text
def transform(text_file_contents):
    return text_file_contents.replace("=", ",")
...
# Dự đoán kết quả cho một mẫu đơn lẻ
@app.route('/predict', methods=['POST', 'GET'])
def predict():
    if request.method == 'POST':
        int_features = [float(x) for x in request.form.values()]
        ...
        # output = prediction[0]
        # if output == 1:
        #     prediction_text = "Sorry to say that you may have heart disease in upcoming 10 years. Do consult your family doctor and take a proper treatment."
        # else:
        #     prediction_text = "Congratulations, you don't have any chances of heart disease in upcoming 10 years and continue your diet and have a good health."
        # return render_template('Individual-Test.html', prediction_text=prediction_text)

    return 0

# Đọc tham số từ config.json
with open('config.json', 'r') as c:
    params = json.load(c)["params"]

# Kết nối với cơ sở dữ liệu SQLAlchemy
app.config['UPLOAD_FOLDER'] = params['upload_location']
app.config['SQLALCHEMY_DATABASE_URI'] = params['local_uri'] if True else params['prod_uri']
db = SQLAlchemy(app)

# Định nghĩa bảng Contact trong cơ sở dữ liệu
class Contact(db.Model):
    srno = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String, nullable=False)
    email = db.Column(db.String, nullable=False)
...
# Chức năng tải và lưu dataset từ file CSV vào MySQL
def parseCSV(filePath, mycursor):
    col_names = ['male', 'age', 'cigsPerDay', 'BPMeds', 'prevalentStroke', 'prevalentHyp', 'diabetes', 'totChol',
                 'sysBP', 'diaBP', 'BMI', 'heartRate', 'glucose']
    csvData = pd.read_csv(filePath, names=col_names, header=None)
...
# Chức năng tải file lên và xử lý
@app.route("/uploader", methods=['GET', 'POST'])
def uploader():
    if request.method == 'POST':
        f = request.files['file']
        ...
        f.save(file_path)
        parseCSV(file_path, mycursor)
        return render_template('upload.html', params=params)

# Chức năng thu thập feedback liên hệ từ người dùng
@app.route("/contact", methods=['GET', 'POST'])
def contact():
    if request.method == 'POST':
        name = request.form.get('name')
...
# Chức năng dự đoán kết quả cho dataset từ file
@app.route('/transform', methods=["POST"])
def transform_view():
    f = request.files['data_file']
    if not f:
    ...
    # Dự đoán kết quả
    # df['TenYearCHD'] = model.predict(df)
