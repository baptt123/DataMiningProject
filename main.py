...
def login():
    if request.method == 'POST':
        ...
        if user:
            ...
            return redirect(url_for('index'))  # Điều hướng tới trang index sau khi đăng nhập thành công
        else:
            # Đăng nhập thất bại
            flash('Thông tin đăng nhập không đúng', 'danger')
            return redirect(url_for('login'))  # Đổi 'login_page' thành 'login'
...
def load_data_from_db():
    ...
    try:
        ...
        if not result:
            print("Không có dữ liệu trong cơ sở dữ liệu.")
            return None
